...
flowchart TB
    ...
    subgraph L1["L1 采集层 · collectors/"]
        ...
        A3b["FeishuScheduleCollector ✅"]
        A3c["DingTalkScheduleCollector ✅"]
        A4["其他 Collector 🔜"]
    end
...
| 飞书日程 | P1 | `feishu_schedule.py` | ✅ | CalDAV → `caldav.feishu.cn` |
| 钉钉日程 | P1 | `dingtalk_schedule.py` | ✅ | CalDAV → `caldav.mxhichina.com` |
...
- 凭证：飞书桌面端「设置 → 日历 → CalDAV 同步」生成的专用用户名/密码
- 发现：标准 CalDAV `calendar-home-set`（`provider=feishu`）；失败时可填 `calendar_id`
- 实现：`feishu_schedule.py` → `calendar/caldav_client.py`
- 用户配置见 [飞书日程采集-用户指南.md](./飞书日程采集-用户指南.md)

### 5.5 钉钉 CalDAV 要点

- 服务器：`https://caldav.mxhichina.com`
- 凭证：PC 端钉钉「日历 → 设置 → CalDAV 同步」生成的专用用户名/密码
- 发现：标准 CalDAV `calendar-home-set`（`provider=dingtalk`）；失败时可填 `calendar_id`
- 实现：`dingtalk_schedule.py` → `calendar/caldav_client.py`
- 用户配置见 [钉钉日程采集-用户指南.md](./钉钉日程采集-用户指南.md)
...
| 字段 | 说明 |
|------|------|
| `source` | `git` / `manual` / `wecom` / `feishu` / `dingtalk` / `chat` / `docs` / `issue` |
...
| Phase 3 | 飞书 CalDAV、钉钉日程  ✅ 、聊天、文档、PR/Issue | 🔜 |
